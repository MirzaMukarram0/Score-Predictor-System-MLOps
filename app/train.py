...
def predict_example(model):
    """Return one-sample prediction for a quick sanity check."""
...
def main():
    parser = argparse.ArgumentParser(
        description="Train linear regression for Student Performance"
    )
    parser.add_argument(
        "--data",
        default="../datasets/StudentPerformanceFactors.csv",
        help=(
            "Path to CSV dataset "
            "(default: ../datasets/StudentPerformanceFactors.csv)"
        ),
    ...
    parser.add_argument(
        "--out",
        default="../datasets/student_model.pkl",
        help=(
            "Path to save trained model pickle "
            "(default: ../datasets/student_model.pkl)"
        ),
    )
    ...
    # Example prediction (kept short to satisfy flake8)
    pred = predict_example(model)
    print("\nPredicted Exam Score for sample example:")
    print(pred)
